...
async def test_place_and_cancel():
    ...
    order = await client.place_maker_order(
        token_id="test-token", side="BUY", price=0.92, size_usdc=50.0,
    )
...
async def test_cancel_all():
    ...
    for i in range(5):
        await client.place_maker_order(
            token_id=f"token-{i}", side="BUY", price=0.92, size_usdc=50.0,
        )
...
async def test_get_open_orders_filtered():
    ...
    await client.place_maker_order("token-a", "BUY", 0.92, 50.0)
    await client.place_maker_order("token-b", "BUY", 0.92, 50.0)
    await client.place_maker_order("token-a", "BUY", 0.93, 30.0)
...
async def test_summary():
    ...
    await client.place_maker_order("t", "BUY", 0.92, 50.0)
    order = await client.place_maker_order("t", "BUY", 0.93, 50.0)
